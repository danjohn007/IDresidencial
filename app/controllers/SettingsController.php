...
class SettingsController extends Controller {
    ...
    public function support() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $supportSettings = [
                ...
                'support_phone' => $this->post('support_phone'),
                'support_hours' => $this->post('support_hours'),
                'support_url' => $this->post('support_url')
            ];
            ...
            $_SESSION['success_message'] = 'Configuración de soporte actualizada';
            $this->redirect('settings/support');
        }
        
        // Obtener configuración actual
        $stmt = $this->db->query("SELECT * FROM system_settings WHERE setting_key LIKE 'support_%'");
        $currentSettings = [];
        while ($row = $stmt->fetch()) {
            $currentSettings[$row['setting_key']] = $row['setting_value'];
        }
        ...
        $data['current'] = $currentSettings;
        $this->view('settings/support', $data);
    }
    
    /**
     * Configuración de Auto-Optimización del Sistema
     */
    public function optimization() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $this->redirect('settings/optimization');
        }
        ...
        $data['current'] = $currentSettings;
        $data['stats'] = $stats;
        $this->view('settings/optimization', $data);
    }
    ...
    private function runOptimizations() {
        try {
            ...
            $tables = ['users', 'residents', 'properties', 'visits', 'access_logs', 
                      'maintenance_fees', 'financial_movements', 'reservations'];
